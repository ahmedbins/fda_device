    ...
    <main>
      ...
      {selected && <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}>
        <aside className="drawer" aria-label="FCC Authorization Dossier">
          ...
          <section className="detail-section"><h3><Database size={16} /> Evidence / source</h3><dl className="fcc-detail-list"><div><dt>Regulatory source</dt><dd>{FCC_SOURCE_LABEL}</dd></div><div><dt>Source mode</dt><dd>{selected.sourceMode === "official_snapshot" ? "Official EAS snapshot" : selected.sourceMode === "official_import" ? "Imported official EAS response" : "Live FCC response"}</dd></div>{selected.snapshotCapturedAt && <div><dt>Snapshot captured</dt><dd>{new Date(selected.snapshotCapturedAt).toLocaleString([], dateTimeFormat)}</dd></div>}<div><dt>Loaded in app</dt><dd>{new Date(selected.retrievedAt).toLocaleString([], dateTimeFormat)}</dd></div></dl><a className="primary official-record-link" href={FCC_SEARCH_URL} target="_blank" rel="noreferrer" onClick={(event) => { event.preventDefault(); openFccEasRecord(selected.fccId); }}>Open this FCC ID in EAS <ExternalLink size={14} /></a><a className="secondary official-record-link" href={selected.sourceUrl} target="_blank" rel="noreferrer">View raw API response <ExternalLink size={14} /></a></section>
