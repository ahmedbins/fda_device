...
export default function FccMonitorPage() {
  ...
  const update = useCallback(async (force = false, nextScopes = scopes) => {
    ...
    try {
      ...
      if (next.unresolvedScopes.length) setCoverageNote(`${next.unresolvedScopes.join(", ")} is outside the FCC scopes this site keeps current. Open the official FCC response from Explorer and import it there to analyze that scope.`);
      setStatus("done");
    } catch (caught) {
      if (controller.signal.aborted) return;
      setRecords([]);
  ...
  return (
    <main>
      ...
      <section className="monitor-controls" aria-label="FCC monitoring controls">
        ...
        <small className="monitor-refreshed">Source: {FCC_SOURCE_LABEL}{searchMeta?.dataAsOf ? ` · FCC data as of ${new Date(searchMeta.dataAsOf).toLocaleString([], dateTimeFormat)}` : ""}{retrievedAt ? ` · pulled ${retrievedAt.toLocaleString([], dateTimeFormat)}` : ""} · activity is based on FCC grant dates, not change detection between refreshes</small>
      </section>
